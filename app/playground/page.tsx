    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center">
      <div className="w-full max-w-6xl mx-auto px-4 py-8 animate-pulse">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1 bg-gray-800 rounded-xl p-4 h-96">
...
function PlaygroundContent() {
  ...
  const runCode = useCallback(() => {
    ...
    if (cssFile && cssFile.content) {
      ...
      htmlContent = htmlContent.replace(/<link[^>]*rel="stylesheet"[^>]*>/gi, styleTag);
    }
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="bg-gray-800 rounded-2xl p-8 max-w-md w-full text-center border border-gray-700">
          ...
          <Link href="/auth/signin" className="inline-block px-6 py-3 bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition">
            Se connecter
          </Link>
        </div>
      </div>
    ...
    <div className={`min-h-screen bg-gray-900 text-white flex flex-col ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
      ...
      <header className="bg-gray-800 border-b border-gray-700 px-3 sm:px-4 py-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
          ...
          <button 
            onClick={() => {
              ...
              if (available.length > 0) {
                const fileType = available[0];
                const nameMap = { html: 'index.html', css: 'style.css', javascript: 'script.js' };
                const contentMap = {
                ...
                const newFile: File = {
                  id: `${fileType}_${generateId()}`,
                  name: nameMap[fileType],
                  type: fileType,
                  language: fileType,
                  content: contentMap[fileType]
                };
                setProjects(prev => prev.map(p => {
                  if (p.id === currentProjectId) {
          ...
          <button onClick={copyCode} className="p-1.5 hover:bg-gray-700 rounded-lg transition text-gray-400 hover:text-white" title="Copier le code">
            <Copy className="w-4 h-4" />
          </button>
          <button onClick={downloadFile} className="p-1.5 hover:bg-gray-700 rounded-lg transition text-gray-400 hover:text-white" title="Télécharger">
            <Download className="w-4 h-4" />
          </button>
          <button onClick={() => setShowDeleteProject(currentProjectId)} className="p-1.5 hover:bg-red-500/20 rounded-lg transition text-gray-400 hover:text-red-400" title="Supprimer le projet">
            <Trash2 className="w-4 h-4" />
          </button>
          <button onClick={toggleFullscreen} className="p-1.5 hover:bg-gray-700 rounded-lg transition text-gray-400 hover:text-white" title={isFullscreen ? 'Quitter plein écran' : 'Plein écran'}>
            {isFullscreen ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}
          </button>
          <button onClick={() => setIsDark(!isDark)} className="p-1.5 hover:bg-gray-700 rounded-lg transition text-gray-400 hover:text-white">
            {isDark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </button>
          <button onClick={runCode} disabled={isRunning} className="flex items-center gap-1 px-2 sm:px-3 py-1.5 bg-green-600 hover:bg-green-700 disabled:opacity-50 rounded-lg transition text-xs sm:text-sm font-medium">
              ...
              {currentProject.files.length > 1 && (
                <button
                  onClick={(e) => { e.stopPropagation(); 
                    ...
                    setProjects(prev => prev.map(p => {
                      if (p.id === currentProjectId) {
                        return { ...p, files: remaining, updatedAt: new Date().toISOString() };
                      }
                      return p;
                    }));
      ...
      <div className="bg-gray-800 border-t border-gray-700 px-3 py-1.5 text-[10px] text-gray-500 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-3">
          ...
          <span className="hidden sm:inline">⌘+S</span><span className="hidden sm:inline">Download</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setFontSize(Math.max(10, fontSize - 2))} className="hover:text-white transition">A-</button>
          <span className="text-xs">{fontSize}px</span>
          <button onClick={() => setFontSize(Math.min(24, fontSize + 2))} className="hover:text-white transition">A+</button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[300] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowNewProjectModal(false)}>
          <div className="bg-gray-800 rounded-2xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button onClick={() => { setShowNewProjectModal(false); setNewProjectName(''); }} className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 transition">
                Annuler
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[300] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowDeleteProject(null)}>
          <div className="bg-gray-800 rounded-2xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowDeleteProject(null)} className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 transition">
                Annuler
              </button>
            </div>
          </div>
