...
import {
  ...
  CheckCheck, GraduationCap, Sparkles, Zap, Database
} from 'lucide-react';
      ...
      return (
        <div key={index} className="mb-8">
          ...
          {block.visualSuggestion && (
            <div className="mt-3 text-sm text-gray-400 dark:text-gray-500 flex items-center gap-2 border-l-2 border-emerald-400 pl-3">
              <Sparkles className="w-4 h-4 text-emerald-400" /> {block.visualSuggestion}
            </div>
          )}
        </div>
      );
      ...
      return (
        <div key={index} className="mb-8">
          <div className="relative rounded-xl overflow-hidden bg-[#0a0a0f] border border-gray-800">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-emerald-500 via-emerald-400 to-green-400 shadow-[0_0_20px_rgba(16,185,129,0.5)]" />
    ...
    case 'table-comparison':
      return (
        <div key={index} className="mb-8 overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
          {block.caption && <h4 className="text-sm font-semibold text-gray-600 dark:text-gray-400 px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700"><Code className="w-4 h-4 inline mr-2 text-emerald-500" />{block.caption}</h4>}
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-700">
                {block.rows.map((row: any[], i: number) => (
                  <tr key={i} className="hover:bg-emerald-50/20 dark:hover:bg-emerald-900/10 transition-colors">
      ...
      return (
        <div key={index} className="mb-8 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden bg-white dark:bg-gray-900">
          <div className="bg-gray-50 dark:bg-gray-800/50 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
            <h4 className="text-sm font-bold text-gray-800 dark:text-gray-200 flex items-center gap-2"><Zap className="w-4 h-4 text-emerald-500" /> {block.title}</h4>
...
const ExerciseRenderer = ({ exercise }: { exercise: any }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 md:p-6 mb-4 shadow-[0_4px_20px_rgba(0,0,0,0.02)] transition-all hover:shadow-[0_8px_30px_rgba(16,185,129,0.06)] hover:border-emerald-200/60">
      <div className="flex items-start gap-3 md:gap-4">
        <div className="flex-shrink-0 mt-1">
          {exercise.type === 'multiple-choice' && <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center text-white shadow-[0_4px_15px_rgba(16,185,129,0.3)]"><Circle className="w-4 h-4 md:w-5 md:h-5" /></div>}
          ...
          {exercise.type === 'code-practice' && <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center text-white shadow-[0_4px_15px_rgba(16,185,129,0.3)]"><Code className="w-4 h-4 md:w-5 md:h-5" /></div>}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {exercise.type === 'multiple-choice' && exercise.options && (
            <div className="space-y-2 mb-4">
              {exercise.options.map((opt: string, idx: number) => {
                ...
                let className = 'block p-3 rounded-xl border-2 cursor-pointer transition-all bg-white dark:bg-gray-800 text-sm hover:border-emerald-300 dark:hover:border-emerald-700';
                if (submitted) {
                  ...
                  className += isSelected ? ' border-emerald-500 shadow-[0_0_25px_rgba(16,185,129,0.15)]' : ' border-gray-200 dark:border-gray-700';
                }
                return (
          ...
          {exercise.type === 'true-false' && (
            <div className="flex gap-2 md:gap-3 mb-4">
              {['Vrai', 'Faux'].map((val) => {
                ...
                let className = 'flex-1 px-4 py-2 rounded-xl border-2 font-semibold transition-all bg-white dark:bg-gray-800 text-sm text-center hover:border-emerald-300 dark:hover:border-emerald-700';
                if (submitted) {
                  if (isCorrectAnswer) className += ' border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 shadow-[0_0_25px_rgba(16,185,129,0.15)]';
                  else if (isSelected && !isCorrectAnswer) className += ' border-red-400 bg-red-50 dark:bg-red-900/20 shadow-[0_0_25px_rgba(239,68,68,0.1)]';
                  else className += ' border-gray-200 dark:border-gray-700 opacity-60';
          ...
          {exercise.type === 'fill-blank' && (
            <div className="mb-4">
              <input type="text" className="w-full p-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 text-sm focus:border-emerald-500 focus:shadow-[0_0_25px_rgba(16,185,129,0.15)] transition outline-none" placeholder="Tapez votre réponse..." value={selected || ''} onChange={(e) => !submitted && setSelected(e.target.value)} disabled={submitted} />
            </div>
          )}
          ...
          {exercise.type === 'code-practice' && (
            <div className="mb-4">
              ...
              <textarea className="w-full p-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 font-mono text-sm placeholder-gray-400 focus:border-emerald-500 focus:shadow-[0_0_25px_rgba(16,185,129,0.15)] transition outline-none resize-none" rows={4} placeholder="Écrivez votre code SQL ici..." value={codeValue} onChange={(e) => !submitted && setCodeValue(e.target.value)} disabled={submitted} />
            </div>
          )}

          {exercise.hint && (
            <div className="mb-4">
              <button className="text-sm text-emerald-600 hover:text-emerald-800 transition flex items-center gap-1.5 font-medium" onClick={() => setShowHint(!showHint)}>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-4">
            {!submitted && isAnswered() && (
              <button className="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white text-sm font-semibold rounded-xl transition shadow-[0_4px_20px_rgba(16,185,129,0.3)] hover:shadow-[0_8px_30px_rgba(16,185,129,0.4)]" onClick={handleSubmit}>
                Soumettre
              </button>
            )}
            {submitted && (
          ...
          {submitted && exercise.explanation && (
            <div className="mt-4 p-4 bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-950/30 dark:to-green-950/30 border border-emerald-200 dark:border-emerald-800 rounded-xl">
              <p className="text-sm text-gray-700 dark:text-gray-300"><span className="font-bold text-emerald-800 dark:text-emerald-400">Explication :</span> {exercise.explanation}</p>
            </div>
          )}
        </div>
      </div>
...
export default function SqlLessonViewer() {
      ...
      <header className="sticky top-0 z-30 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-800/50 shadow-sm">
        <div className="flex items-center justify-between px-3 sm:px-4 md:px-6 py-3 md:py-4 max-w-7xl mx-auto">
          <div className="flex items-center gap-2 md:gap-4 min-w-0">
            <Link href="/cours" className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1.5 md:py-2 text-xs md:text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-950/30 transition flex-shrink-0">
            ...
            <div className="flex items-center gap-2 md:gap-3 min-w-0">
              <div className="w-7 h-7 md:w-10 md:h-10 rounded-xl md:rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-[0_4px_20px_rgba(16,185,129,0.4)] flex-shrink-0">
                <Database className="w-3.5 h-3.5 md:w-5 md:h-5 text-white" />
              </div>
              <div className="min-w-0">
                <h1 className="text-xs sm:text-sm md:text-base font-extrabold bg-gradient-to-r from-emerald-500 to-emerald-600 bg-clip-text text-transparent tracking-tight truncate">SQL & BD</h1>
          ...
          <div className="flex items-center gap-2 md:gap-4 text-xs md:text-sm flex-shrink-0">
            ...
            <div className="w-16 sm:w-20 md:w-32 h-1.5 bg-gray-200/80 dark:bg-gray-700/80 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-emerald-500 to-emerald-600 transition-all duration-500" style={{ width: `${progress}%` }} />
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="flex flex-1 max-w-7xl mx-auto w-full relative">
        ...
        <aside className={`fixed inset-0 z-40 lg:relative lg:inset-auto lg:z-0 w-[280px] sm:w-[320px] lg:w-72 bg-white dark:bg-gray-900 border-r border-gray-200/80 dark:border-gray-800/50 transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 flex flex-col overflow-y-auto shadow-2xl lg:shadow-none`}>
          <div className="p-4 md:p-5 border-b border-gray-200/80 dark:border-gray-800/50 flex items-center justify-between lg:justify-start">
            <h2 className="font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2.5 text-xs md:text-sm tracking-wide uppercase">
              <List className="w-4 h-4 text-emerald-500" /> Chapitres
            </h2>
                ...
                {chapter.lessonIds.map((id) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 md:py-2.5 rounded-xl md:rounded-2xl text-xs md:text-sm transition-all flex items-center gap-3 ${isActive ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 font-semibold border border-emerald-200/40 dark:border-emerald-800/30' : 'hover:bg-gray-50 dark:hover:bg-gray-800/50 text-gray-600 dark:text-gray-400'}`}
                    >
                      <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full flex-shrink-0 ${isActive ? 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.6)] animate-pulse' : 'bg-gray-300 dark:bg-gray-600'}`} />
                      <span className="truncate text-xs md:text-sm">{l.title}</span>
                      {l.exercises.length > 0 && <span className={`ml-auto text-[9px] md:text-[10px] font-bold px-1.5 md:px-2 py-0.5 rounded-full ${isActive ? 'bg-emerald-200/80 text-emerald-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'}`}>{l.exercises.length}</span>}
                    </button>
                  );
                })}
              </div>
          ...
          <div className="p-3 md:p-4 border-t border-gray-200/80 dark:border-gray-800/50 text-[10px] md:text-xs text-gray-400 dark:text-gray-500 flex items-center justify-between">
            ...
            <GraduationCap className="w-3.5 h-3.5 md:w-4 md:h-4 text-emerald-400" />
          </div>
        </aside>
        ...
        <main className="flex-1 w-full min-w-0 px-3 sm:px-6 md:px-10 lg:px-14 py-4 md:py-10">
          ...
          {lesson.keyTakeaways && lesson.keyTakeaways.length > 0 && (
            <div className="mt-8 md:mt-12 p-4 md:p-6 bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-950/30 dark:to-green-950/30 border border-emerald-200 dark:border-emerald-800 rounded-2xl">
              <h3 className="font-extrabold text-emerald-800 dark:text-emerald-400 flex items-center gap-2 md:gap-3 text-base md:text-lg">
                <Lightbulb className="w-5 h-5 md:w-6 md:h-6 text-emerald-500" /> Points clés
              </h3>
          ...
          <div className="mt-12 md:mt-16 flex flex-wrap items-center justify-between gap-3 md:gap-4 border-t border-gray-200/80 dark:border-gray-800/50 pt-6 md:pt-8">
            ...
            <div>
              {nextId ? (
                <button onClick={() => setCurrentLessonId(nextId)} className="inline-flex items-center gap-1.5 md:gap-2 px-6 md:px-8 py-2.5 md:py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white text-xs md:text-sm font-semibold rounded-xl md:rounded-2xl transition shadow-[0_4px_20px_rgba(16,185,129,0.3)] hover:shadow-[0_8px_30px_rgba(16,185,129,0.4)]">
